...
(function() {
    // -------------------------------------------------------------------------
    //  CORE LOGIC: This function checks the URL and applies changes.
    // -------------------------------------------------------------------------
    function adjustLanguageAndDirection() {
        ...
        if (isWarpScannerPage) {
            // --- This is a Warp Scanner (Persian) page ---
            if (htmlTag.getAttribute('lang') !== 'fa') {
                console.log("Switching to FA/RTL for Warp Scanner page.");
                htmlTag.setAttribute('lang', 'fa');
                htmlTag.setAttribute('dir', 'rtl');
                
                // Inject the强制 RTL style if it doesn't exist
                if (!document.getElementById(styleId)) {
                    const style = document.createElement('style');
                    style.id = styleId;
                    style.innerHTML = `[dir="rtl"] * { direction: rtl !important; }`;
                    document.head.appendChild(style);
                }
            }
        } else {
            // --- This is any other (English) page ---
            if (htmlTag.getAttribute('lang') !== 'en') {
                console.log("Switching to EN/LTR for default page.");
                htmlTag.setAttribute('lang', 'en');
                htmlTag.setAttribute('dir', 'ltr');

                // Remove the RTL override style if it exists
                const rtlStyle = document.getElementById(styleId);
                if (rtlStyle) {
                    rtlStyle.remove();
                }
            }
    ...
    // -------------------------------------------------------------------------
    //  TRIGGER LOGIC: This ensures the function runs on every page change.
    // -------------------------------------------------------------------------
    ...
    // 2. Use MutationObserver to detect navigation changes (the key part!)
    // The 'md-container' is a stable element that holds the page content.
    const container = document.querySelector(".md-container");
    if (container) {
        const observer = new MutationObserver(() => {
            // This code runs EVERY time the content inside the container changes.
            // We need a small delay because the title/URL might not be updated yet.
            setTimeout(() => {
                console.log("Instant navigation detected: re-applying language settings.");
                adjustLanguageAndDirection();
            }, 100); // A 100ms delay is usually safe
        });

        // We observe the 'md-container' for changes in its child elements.
        // When a new page is loaded, its children (like md-main) will be replaced.
