...
//# sourceMappingURL=custom.9e5da760.min.js.map

function adjustLanguageAndDirection() {
    const currentPath = window.location.pathname;
    const isWarpScannerPage = currentPath.includes('/warp-scanner/');
    const htmlTag = document.documentElement; // This is the <html> tag

    if (isWarpScannerPage) {
        // --- This is a Warp Scanner page ---

        // 1. Set the language attribute of the <html> tag to "fa"
        htmlTag.setAttribute('lang', 'fa');
        
        // 2. Set the direction of the <html> tag to "rtl"
        //    We also set it on the body for extra safety, though html is usually enough.
        htmlTag.setAttribute('dir', 'rtl');
        document.body.setAttribute('dir', 'rtl');

        // 3. Inject a CSS style block to force RTL on everything
        //    This is a powerful override for any lingering LTR issues.
        const styleId = 'rtl-override-style';
        if (!document.getElementById(styleId)) {
            const style = document.createElement('style');
            style.id = styleId;
            // The CSS rule you requested. It forces all elements within a page
            // with `dir="rtl"` to obey the right-to-left direction.
            style.innerHTML = `
                [dir="rtl"] * {
        ...
        console.log("Warp Scanner page detected. Switched to RTL mode.");

    } else {
        // --- This is any other page (e.g., python-v2ray) ---

        // 1. Ensure the language is set to "en"
        htmlTag.setAttribute('lang', 'en');
        
        // 2. Ensure the direction is set to "ltr"
        htmlTag.setAttribute('dir', 'ltr');
        document.body.setAttribute('dir', 'ltr');
        
        // 3. If the override style exists from a previous page, remove it.
        const rtlStyle = document.getElementById('rtl-override-style');
        if (rtlStyle) {
            rtlStyle.remove();
        }
        console.log("Default page detected. Switched to LTR mode.");
    }
}

// --- Logic to run the function on page changes ---

// Material for MkDocs uses instant navigation (PJAX/AJAX) to load pages
// without a full refresh. A simple 'DOMContentLoaded' is not enough.
// We need to listen to their custom events.

// The 'md-content' element is where the new page content is loaded.
const contentElement = document.querySelector(".md-content");

if (contentElement) {
    // Create a MutationObserver to watch for when the content changes.
    // This is the most reliable way to detect page navigation.
    const observer = new MutationObserver((mutations) => {
        // We don't need to check the mutations themselves.
        // The fact that the content changed is enough.
        console.log("Page content changed, re-applying language settings.");
        adjustLanguageAndDirection();
    });

    // Start observing the 'md-content' element for any changes to its children.
...
// Also run it once on the initial page load, just in case.
document.addEventListener('DOMContentLoaded', () => {
    console.log("Initial page load, applying language settings.");
    adjustLanguageAndDirection();
});
