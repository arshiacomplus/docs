...
# *** تغییر: مسیر فایل Markdown نسبت به ریشه ریپازیتوری ***
MARKDOWN_FILE_PATH = Path('docs/setup.md') # <<< مطمئن شوید این مسیر از ریشه درست است
...
PLACEHOLDERS = {
    ...
    "{{ WIN7_PORTABLE_LINK }}": "#", "{{ WIN7_PORTABLE_FILENAME }}": "یافت نشد", "{{ WIN7_PORTABLE_VERSION }}": "نامشخص",
    "{{ WIN8_UPPER_PORTABLE_LINK }}": "#", "{{ WIN8_UPPER_PORTABLE_FILENAME }}": "یافت نشد", "{{ WIN8_UPPER_PORTABLE_VERSION }}": "نامشخص",
    "{{ WIN7_SETUP_LINK }}": "#", "{{ WIN7_SETUP_FILENAME }}": "یافت نشد", "{{ WIN7_SETUP_VERSION }}": "نامشخص",
    "{{ WIN8_UPPER_SETUP_LINK }}": "#", "{{ WIN8_UPPER_SETUP_FILENAME }}": "یافت نشد", "{{ WIN8_UPPER_SETUP_VERSION }}": "نامشخص",
    "{{ LINUX_LINK }}": "#", "{{ LINUX_FILENAME }}": "یافت نشد", "{{ LINUX_VERSION }}": "نامشخص", "{{ LINUX_NOTE }}": "خطا در بررسی نسخه لینوکس.",
    "{{ ANDROID_LATEST_VERSION }}": "نامشخص", "{{ ANDROID_APK_LINK }}": "#", "{{ ANDROID_APK_FILENAME }}": "یافت نشد", "{{ ANDROID_APK_VERSION }}": "نامشخص",
}
...
def find_asset(assets, filename_part):
    ...
    for asset in assets:
        if filename_part in asset.get('name', ''):
            return {'url': asset.get('browser_download_url'), 'name': asset.get('name')}
    return None
...
if __name__ == "__main__":
    ...
    # دریافت اطلاعات مخزن اصلی... (بدون تغییر در منطق fetch و پردازش)
    ...
    if main_release:
        ...
        win7p = find_asset(assets, 'win7-Portable.rar'); win8p = find_asset(assets, 'win8-upper-Portable.rar'); win7s = find_asset(assets, 'win7-Setup.exe'); win8s = find_asset(assets, 'win8-upper-Setup.exe')
        ...
        if linux_asset:
            PLACEHOLDERS["{{ LINUX_LINK }}"] = linux_asset['url']; PLACEHOLDERS["{{ LINUX_FILENAME }}"] = linux_asset['name']; PLACEHOLDERS["{{ LINUX_VERSION }}"] = main_version
            ...
            PLACEHOLDERS["{{ LINUX_LINK }}"] = LINUX_FALLBACK_URL; PLACEHOLDERS["{{ LINUX_FILENAME }}"] = LINUX_FALLBACK_FILENAME; PLACEHOLDERS["{{ LINUX_VERSION }}"] = LINUX_FALLBACK_VERSION
        ...
        print("Failed to fetch main repository data.", file=sys.stderr)

    # دریافت اطلاعات مخزن اندروید... (بدون تغییر در منطق fetch و پردازش)
    android_release = fetch_latest_release(ANDROID_REPO)
    android_version = "خطا"
    if android_release:
        android_version = android_release.get('tag_name', 'نامشخص')
        PLACEHOLDERS["{{ ANDROID_LATEST_VERSION }}"] = android_version; PLACEHOLDERS["{{ ANDROID_APK_VERSION }}"] = android_version
        assets = android_release.get('assets')
        apk_asset = find_asset(assets, '.apk')
        ...
        print("Failed to fetch Android repository data.", file=sys.stderr)


    # --- خواندن، جایگزینی و نوشتن فایل Markdown ---
    # *** تغییر: استفاده مستقیم از MARKDOWN_FILE_PATH ***
    markdown_file_path = MARKDOWN_FILE_PATH

    # *** افزودن بررسی وجود فایل با جزئیات بیشتر ***
    if not markdown_file_path.is_file():
        print(f"Error: Markdown file not found at expected path '{markdown_file_path}' relative to the current working directory.", file=sys.stderr)
        print(f"Current Working Directory: {Path.cwd()}", file=sys.stderr)
        print("Files/Dirs in CWD:", [p.name for p in Path.cwd().iterdir()], file=sys.stderr)
        # سعی کنید مسیر docs را هم لیست کنید اگر وجود دارد
        docs_dir = Path('docs')
        if docs_dir.is_dir():
             print("Files/Dirs in ./docs:", [p.name for p in docs_dir.iterdir()], file=sys.stderr)
